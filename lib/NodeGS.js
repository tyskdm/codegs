...
function require(firstModulePath) {
    ...
    exports.require = function (modulePath, option) {
        if (modulePath === nodeGsName) {
            return chache[modulePath].exports;
        }

        var path = exports.resolve(modulePath);
        if (!chache[path]) {
            return chache[path].exports;
        }

        if (moduleFactories[path]) {
            var self = moduleFactories[path];
            var gsModule = new GsModule(path, PARENT, path);

            chache[path] = gsModule;
            self.module = gsModule;
            REQUIRE = function (id) {
                return self.require(id);
            };

            moduleFactories[path].func(
                gsModule.exports,
                REQUIRE,
                gsModule,
                __filename,
...
if (module && module.exports && exports && __filename && __dirname) {
    module.exports = require(nodeGsName); // ?????
