...
Codegs.prototype.setup = function (config, mockfs) {
    ...
    this.files.main = joinPath(config.rootdir, config.mainfile);
...
Codegs.prototype.addCoreFiles = function (mockfs) {
    ...
    var coredir = joinPath(this.config.rootdir, this.config.core);
...
Codegs.prototype.addNodeCoreFiles = function (mockfs) {
    ...
    var nodecoredir = joinPath(this.config.rootdir, this.config.node_core);
...
Codegs.prototype.addSourceFiles = function (mockfs) {
    ...
    for (i = 0; i < this.config.source.length; i++) {
        sourcedir = joinPath(this.config.rootdir, this.config.source[i]);
        storedir = joinPath('/', this.config.source[i]);
...
Codegs._addFilesToList = function (list, dir, basepath, mockfs) {
    ...
    for (i = 0; i < files.length; i++) {
        ...
        if (stat.isDirectory()) {
            error = Codegs._addFilesToList(list, filename, joinPath(basepath, files[i]), mockfs);
            if (error !== null) {
                return error;
            }
            continue;
